...
class RobotChessController:
    def __init__(self):
        ...
        self.calibrated_positions = {
            # Top-left corner (a8)
            'a8': [0, -1.094, -0.290, 0.552, 1.267, 0],  
            # Top-right corner (h8)
            'h8': [0, 1.022, -0.313, 0.537, 1.322, 0],    
            # Bottom-right corner (h1)
            'h1': [0, 0.356, 0.672, -0.833, 1.453, 0],   
            # Bottom-left corner (a1)
            'a1': [0, -0.407, 0.764, -1.015, 1.562, 0]   
        }
    ...
    def calculate_world_positions(self):
        ...
        # Calculate world positions for corners using forward kinematics
        ...
        for rank in ranks:
            for file in files:
                ...
                if square in self.square_centers:
                    ...
                    # Calculate normalized position creating a ratio of the camera position to the camera corners
                    x_norm, y_norm = self._get_normalized_position(camera_pos, camera_corners_array) 
                    
                    # Interpolate world position using the normalized position
                    ...
                    print(f"Mapped {square} to {world_pos}")
    ...
    def generate_move_trajectory(self, uci_move):
        ...
        for i, point in enumerate(trajectory_points):
            # Default orientation (gripper facing down)
            orientation = np.array([0, .7071, 0, .7071])  # Identity quaternion
            
            # For captured piece storage points, use the orientation from forward kinematics
            if uci_move.endswith('x'):
