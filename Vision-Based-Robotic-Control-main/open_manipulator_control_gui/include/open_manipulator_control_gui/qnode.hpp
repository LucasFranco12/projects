...
/* Authors: Darby Lim, Hye-Jong KIM, Ryan Shim, Yong-Ho Na, and further edited by Lucas Franco to recieve chess move trajectory */
...
public:
  ...
  // Declare the chess move callback
  void chessMoveCallback(const std_msgs::String::ConstPtr &msg);
  // Declare the chess move trajectory execution function
  // This function will be called when a chess move is received
  // It will parse the move and execute the corresponding trajectory
  void executeJointTrajectory(const std::string &trajectory_json);

Q_SIGNALS:
  void rosShutdown();
  void chessMoveReceived(QString move);
...
private:
  ...
  // Declare the chess move subscriber
  ros::Subscriber chess_move_sub_;
