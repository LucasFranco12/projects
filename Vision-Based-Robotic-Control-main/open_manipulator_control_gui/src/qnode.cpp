...
/* Authors: Darby Lim, Hye-Jong KIM, Ryan Shim, Yong-Ho Na, and further edited by Lucas Franco to execture recieved trajectory */
// Edits made by Lucas Franco are the Chess Move Callback and execture joint trajectory functions
/*****************************************************************************
** Includes
*****************************************************************************/
...
// Chess Move Callback
// This function will be called when a chess move is received then calls the executeJointTrajectory function
...
// This executes the joint trajectory based on the received JSON string
// It parses the JSON, extracts joint angles, and sends commands to the robot
// It also handles gripper control based on the trajectory points
// The function assumes the JSON format is an array of arrays, where each inner array contains joint angles
            ...
            if (setJointSpacePath(joint_names, joint_angles, 2.0)) {
                ...
                if (num_points > 6) {  // This is a capture move
                    ...
                    if (i == 1) {  // Close gripper to grab captured piece
                        ...
                        setToolControl({0.01});
                    } else if (i == 7) {  // Close gripper to grab moving piece
                        setToolControl({0});
                    } else if (i == 10) {   // Open gripper to place moving piece
                        setToolControl({0.01});
                    }
                } else {  // Standard move
