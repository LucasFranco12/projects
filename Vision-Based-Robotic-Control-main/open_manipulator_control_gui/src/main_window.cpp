  ...
  , qnode(argc, argv)
{
  ...
  // Connect the chess move signal to the GUI update slot
  QObject::connect(&qnode, SIGNAL(chessMoveReceived(QString)),
                   this, SLOT(updateChessMoveDisplay(QString)));

  // Initialize the QNode
  if (!qnode.init()) {
    QMessageBox::critical(this, "Error", "Failed to initialize ROS node.");
    close();
  }
...
void MainWindow::on_btn_read_joint_angle_clicked(void)
{
  ...
  fp = fopen("/home/l/catkin_ws/angles.txt", "at");
  fprintf(fp, "%.6f ", joint_angle.at(0));
  fprintf(fp, "%.6f ", joint_angle.at(1));
  fprintf(fp, "%.6f ", joint_angle.at(2));
  fprintf(fp, "%.6f ", joint_angle.at(3));
  fprintf(fp, "\n");
  fclose(fp);
...
void MainWindow::updateChessMoveDisplay(const QString &move)
{
  ui.label_chess_move->setText("Chess Move: " + move);
}
